...
export const ConfigSchema = Type.Object({
  ...
  // Declared here rather than behind MODE.dependencies: SignalK's admin UI
  // rebuilds the form schema and copies only `description` and `properties`
  // from a plugin's schema (PluginConfigurationForm.tsx), so a `dependencies`
  // rule never reaches RJSF. Declared there, the field vanished in BOTH
  // toggle positions and a self-hosted URL could not be entered at all.
  // Revisit once the admin UI forwards conditional schemas.
